...
func NewSimulationScheduleAlertRepository() interfaces.ScheduleAlertRepositoryIF {
	return &scheduleAlertRepository{
		con:   connection.GetCon(),
		table: "simulation_schedule_alerts",
	}
}
	...
	WITH all_operation_settings AS (
		SELECT
			simulation_operation_settings.id
			 ...
			 , simulation_operation_settings.facility_id
		FROM
		...
		FROM simulation_facilities f
				 INNER JOIN simulation_gantt_groups gg on f.id = gg.facility_id
				 INNER JOIN simulation_units u on u.id = gg.unit_id
				 INNER JOIN simulation_tickets t on gg.id = t.gantt_group_id
				 INNER JOIN simulation_processes p on t.process_id = p.id
...
func (r *scheduleAlertRepository) Find(id int32) db.ScheduleAlert {
	...
	result := r.con.Table(r.table).First(&scheduleAlert, id)
	if result.Error != nil {
		panic(result.Error)
	}
